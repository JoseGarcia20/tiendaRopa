...
<header class="main-header">
  ...
  include ("conexion_servidor.php");
	...
	$datos_cliente=$cliente->seleccionar_cliente($id);
	...
	if(isset($_POST) && !empty($_POST)){
		...
		$res = $cliente->actualizar_cliente($id,$documento,$nombre,$apellidos,$email,$fechaNac,$celular,$telefono,$estilo);
    ...
    <form class="d-flex flex-column p-1 g-3 needs-validation m-3" method="post">
        ...
        <div id="main-header__title" class="p-4 d-flex justify-content-center align-items-center" ><H1>Actualización datos clientes</H1></div>
        
              <div class="formulario">
                <div class="mb-3 form-floating">
                  <input type="hidden" name="id" id="id" class='form-control' value="<?php echo $datos_cliente->dat_id;?>" >
                  <input type="text" class="form-control" id="nombre" name="documento" required value="<?php echo $datos_cliente->dat_documento;?>">
                  <label for="nombre" class="form-label p-3">Documento: </label>
                </div>

              <div class="formulario">
                <div class="mb-3 form-floating">
                  <input type="text" class="form-control" id="nombre" name="nombre" required value="<?php echo $datos_cliente->dat_nombre;?>">
                  <label for="nombre" class="form-label p-3">Nombre Completo: </label>
              </div>

              <div class="mb-3 form-floating">
                  <input type="text" class="form-control" id="apellido" name="apellido" required value="<?php echo $datos_cliente->dat_apellidos;?>" >
                  <label for="apellido" class="form-label p-3">Apellidos: </label>
              </div>

              <div class="mb-3 form-floating">
                  <input type="email" class="form-control" id="email" name="email" required value="<?php echo $datos_cliente->dat_email;?>" >
                  <label for="correo" class="form-label p-3">Email: </label>
              </div>

              <div class="mb-3 form-floating">
                  <input type="date" class="form-control" id="fechaNacimiento" name="fechaNacimiento" required value="<?php echo $datos_cliente->dat_fecha;?>">
                  <label for="fechaNacimiento" class="form-label p-3">Fecha Nacimiento: </label>
              </div>

              <div class="mb-3 form-floating">
                  <input type="number" class="form-control" id="celular" name="Celular" required value="<?php echo $datos_cliente->dat_celular;?>" placeholder="123456">
                  <label for="celular" class="form-label p-3">Celular: </label>
              </div>

              <div class="mb-3 form-floating">
                  <input type="number" class="form-control" id="telefono" name="telefono" required value="<?php echo $datos_cliente->dat_telefono;?>" placeholder="123456">
                  <label for="telefono" class="form-label p-3">Telefono: </label>
              </div>

              <div class="mb-3">
                  <label for="estilo" class="form-label" >¿Algún estilo en especial que te guste? Cuentanos...</label>
                  <textarea class="form-control" id="estilo" name="estilo" rows="2"> <?php echo $datos_cliente->dat_estilo;?> </textarea>
			  </div>
